/* ============================================================
   AU CORE — client modul  (Nivo 0)
   - Čita platform-url.json sa GitHub Pages → zna gde je AU Core server
...
const STORAGE_KEY = 'aucore_url';
const STORAGE_SESSION_KEY = 'aucore_session';
...
async function apiCall(method, path, body) {
  ...
  if (!base) throw new Error('AU Core server nije dostupan');
...
// hubUrl: baza AU Core servera (iz ?hub= URL param)
async function fetchShare(hubUrl, token) {
  ...
  global.AUCore = { getPlatformUrl, apiCall, isServerAvailable, getSession, setSession, syncEvents, createShare, fetchShare };
