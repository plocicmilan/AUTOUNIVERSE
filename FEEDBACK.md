...
| 2 | **Deljena IndexedDB baza između aplikacija na istom origin-u.** Odlučeno: svaka app ima SOPSTVENI namespace u Store-u + sopstveni export podataka. Nema deljenja između Garage/Driver na istom origin-u. | Milan (odluka 2026-07-19) | core (store) | sve | ✅ URAĐENO — `store.web.js`: DB_NAME = "au_" + appName (au_garage / au_driver odvojeni), settings: prefix au_garage_ / au_driver_ |
...
| 20 | **Mehaničar dobija besplatnu reputaciju preko marketplace-a** — vozač na kraju servisa opt-in (default da): "Da li želiš da ovaj servis bude javan na oglasu kada prodaješ vozilo?". Markov naziv u dosijeu svakog vozila koje je servisirao. Kupac zove Marka da potvrdi. Marko dobija reklamu bez plaćanja. | Milan + arhitektura | core (Event.public flag) + hub | Garage + marketplace | ✅ URAĐENO — `models.js`: `public_on_marketplace` (default true) + `mechanic_name`; `workorder.js`: polja u share payload; Driver `hub_import`: toggle "Javno u dosijeu" (default ON) + `importHubRecord()` čita toggle |
| 21 | **Autopijaca — poseban server (port 3001)** — marketplace za prodaju automobila. Driver (trade_mode=true) objavljuje oglas → POST /listings. Javna stranica /prodaja. seller_token autorizacija bez naloga. MVP: LIVE lokalno. | Milan 2026-07-18 | server (autopijaca) | Driver → Autopijaca | ✅ URAĐENO — `autouniverse/autopijaca/`, port 3001 |
| 22 | **Autodelovi — poseban server (port 3002)** — marketplace za prodaju auto delova. Garage objavljuje deo → POST /parts. Javna stranica /delovi. 11 kategorija + kompatibilnost (make/model/year). | Milan 2026-07-18 | server (autodelovi) | Garage → Autodelovi | ✅ URAĐENO — `autouniverse/autodelovi/`, port 3002 |
| 24 | **Trade dashboard — godišnji sažetak prometa** — kolega preprodavac ima 4 aktivna + 5–6 mesečno; nema agregatni prikaz: koliko vozila prodato, prosečan profit, top/loše vozilo, ukupan prihod. Lokalni izračun iz `vehicles` + `events` store-a, nula servera. Ekran: Sažetak / Aktivni / Godišnji. | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.61.0) |
| 25 | **Servisni pasoš PDF za prodaju vozila** — kompletan export iz Driver-a: timeline svih događaja + troškovi + servisi + dokumenti. Prilaže se uz oglas na Autopijaci. jsPDF je već bundled. Format: naslov "DOSIJE VOZILA", header (VIN/marka/model/km), timeline po datumu, sažetak troškova, QR na Autopijaca oglas (opciono). | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.61.0) |
| 26 | **VIN validator + WMI offline lookup** — korisnik unese 17-char VIN → offline match na WMI bazu (World Manufacturer Identifier, prvih 3 znaka) → prikaže marku/državu/tip vozila. Bez servera. WMI tabela: ~200 najčešćih (VW/BMW/Toyota/Ford/Opel/Renault/Peugeot/Fiat...). Integriše se u "Dodaj vozilo" form u Driveru i Garage-u. | Milan 2026-09-13 | core (Vehicle onboarding) | Driver + Garage | ✅ URAĐENO (Driver v1.61.0) — bugfix: window.App = App nije bio global, VIN analiza failovala |
| 27 | **Car Check PDF export** — "Šta proveriti pri kupovini" checklist postoji u Driver-u ali nema PDF export. Generisati PDF sa: datum, ime kupca, vozilo (marka/model/god/VIN/km), 20-tačkovna checklist sa poljem za komentar po tački, polje za potpis i datum. Koristiti jsPDF koji je bundled. | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.49.0) |
| 37 | **Spark plugs tracker** — evidencija zamene svećica: tip (standardne/iridijum/platina), brend, količina, razmak elektrode (mm), interval zamene (km). Screen prikazuje: km od zamene vs interval (color-coded, upozorenje ≥90%), default interval po tipu, istorija sa km-intervalima. | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.59.0) |
| 38 | **Transmission fluid tracker** — evidencija zamene ulja menjača: tip menjača (manuelni/automatik/CVT/DSG/PDK), tip tečnosti, količina (L), brend, interval zamene (km), filter zamenjen (checkbox). Screen prikazuje: km od zamene vs interval (color-coded, upozorenje ≥90%), default interval po tipu menjača, istorija. | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.60.0) |
| 36 | **Coolant tracker** — evidencija zamene rashladne tečnosti: tip (G12+/G13…), koncentracija (%), interval zamene (god.), checkboxi za termostat i pregled cevi. Screen prikazuje: starost vs interval (color-coded, upozorenje ≥90%), istorija. | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.58.0) |
| 35 | **Belts and chains tracker** — evidencija zamene kaišа/lanca razvoda: tip (zupčasti kaiš/lanac/klinasti kaiš), interval zamene (km), checkboxi za vodenu pumpu/zatezač/remenicu. Screen prikazuje: km od zamene vs interval (color-coded, upozorenje ≥90%), istorija. | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.57.0) |
| 34 | **Brake tracker** — evidencija servisa kočnica: debljina pločica napred/nazad (mm), checkboxi za diskove i kočionu tečnost. Novi tip "Kočnice". Screen prikazuje: poslednji servis sa mm debljinom pločica (color-coded, upozorenje <3mm), istorija. | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.56.0) |
| 33 | **Oil change tracker** — evidencija zamene ulja: specifikacija (5W-30), brend, količina (L), checkboxi za filter ulja/vazduha/kabine. Novi tip "Zamena ulja". Screen prikazuje: km i mesece od poslednje zamene (color-coded, upozorenje ≥10.000km ili ≥12mes), istorija sa km-intervalom između zamena. | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.55.0) |
| 32 | **Battery tracker** — evidencija zamene akumulatora: brend, kapacitet (Ah), struja (CCA/A), stanje (novo/polovan/slab/mrtav). Novi tip događaja "Akumulator". Screen prikazuje: aktuelni akumulator sa starošću (color-coded, upozorenje ≥3.5 god), km od ugradnje, istorija zamena. | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.54.0) |
| 31 | **Tire tracker / istorija guma** — evidencija svake zamene guma: set (letnje/zimske/cjelogodišnje), dimenzija, brend, dubina gaznog sloja (mm). Event forma za tip "Gume" dobija dedikovan blok. Screen "Istorija guma" prikazuje: aktuelni set, ukupno zamena, avg km/set, hronološka lista sa tread indikatorom (zeleno/žuto/crveno). | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.53.0) |
| 30 | **Fuel log i statistike potrošnje** — evidencija svakog punjenja goriva: litara, cena/litar (auto-calc ukupno), km pri punjenju. Screen "Evidencija goriva" prikazuje: ukupno litara, avg L/100km (iz km razlike između punjenja), ukupno potrošeno, lista punjenja sa potrošnjom po intervalu. Dugme na Troškovi screenu. | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.52.0) |
| 29 | **Reminder za sledeći servis iz event forme** — pri unosu servisnog događaja, korisnik može da unese "sledeći servis za X km / do datuma". Ako popuni bar jedno polje, automatski se kreira podsetnik. Ista logika kao hub_import, sad i za manuelni unos. | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.51.0) |
| 28 | **Troškovi servisa po mehaničaru/radionici** — agregirani prikaz troškova grupisanih po servisu/radionici za vozilo. Polje "Servis / radionica" u event formi (opciono). Screen prikazuje: naziv servisa, broj poseta, ukupni trošak (RSD/EUR odvojeno), datum poslednje posete. Sortiran po ukupnom trošku. | Milan 2026-09-13 | app (Driver) | Driver | ✅ URAĐENO (Driver v1.50.0) |
| 23 | **Brzi onboarding vozila kroz skener saobraćajne** — posmatranje kod Gorana (Inter Cars app: slika saobraćajnu → VIN + podaci popunjeni automatski). Plan: Offline WMI decoder (F1, i za web PWA) + NHTSA vPIC (F2) + Aztec 2D skener saobraćajne kamerom (F3, samo Capacitor). NFC čip otpisan. Detalji: `ideas/hypothesis/2026-07-22_vin_scanning_onboarding.md`. Uslov aktivacije: Capacitor shell + testerski signal. | Milan (Goran) 2026-07-22 | core (Vehicle onboarding) | Driver + Garage | ⚪ hipoteza — čeka Capacitor + signal |
